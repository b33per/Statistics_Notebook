...
<div id="search-ui">
  <form id="search-form" onsubmit="doSearch(); return false;">
    <input id="q" type="text" placeholder="Search the whole site…" style="width:100%;padding:10px;border:1px solid #ccc;border-radius:8px" />
  </form>
...
<script>
const INDEX_URL = '/Statistics_Notebook/search-index.json?v=3'; // absolute path (your repo name)
let miniSearch = null;
...
function render(results) {
  ...
  ul.innerHTML = results.map(r => {
    ...
    return `<li style="margin:8px 0">
      <a href="${r.url}">${title}</a><br/><small>${r.url}</small>
    </li>`;
  }).join('');
  document.getElementById('status').textContent =
    results.length ? `${results.length} result(s)` : 'No matches';
}
...
async function loadIndex() {
  try {
    ...
    // also lowercase content/title before indexing (extra safe)
    ...
    document.getElementById('status').textContent =
      `Indexed ${docs.length} page(s). Type & press Enter.`;
  } catch (e) {
    document.getElementById('status').textContent =
...
document.getElementById('q').addEventListener('input', () => {
  ...
  if (!v || !miniSearch) return;
  render(miniSearch.search(v, { prefix: true, fuzzy: 0.2 }));
});
