...
<div id="search-ui">
  ...
  <p id="status" style="margin-top:8px;color:#777;">Loading index…</p>
  <ul id="results"></ul>
</div>
...
<script>
// wait until the HTML above exists
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!qEl || !statusEl || !resultsEl) return; // safety
  ...
  function render(results) {
    resultsEl.innerHTML = results.map(r => `
      <li style="margin:8px 0">
        <a href="${r.url}">${r.title || r.url}</a><br/>
        <small>${r.url}</small>
      </li>`).join('');
  ...
  async function loadIndex() {
    try {
      ...
      miniSearch.addAll(docs.map(d => ({
        title: (d.title||"").toLowerCase(),
        url: d.url,
        content: (d.content||"").toLowerCase()
      })));

      statusEl.textContent = `Indexed ${docs.length} page(s). Type & press Enter.`;
    } catch (e) {
      statusEl.textContent = `Could not load search index (${e.message}).`;
  ...
  function doSearchNow() {
    ...
    if (!miniSearch || !query) { render([]); return; }
    render(miniSearch.search(query, { prefix: true, fuzzy: 0.2 }));
  }

  qEl.addEventListener('input', doSearchNow);
  qEl.form.addEventListener('submit', (e) => { e.preventDefault(); doSearchNow(); });
