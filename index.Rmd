...
<div id="search-ui">
  ...
  <p id="count" style="margin-top:8px;"></p>
  <ul id="results"></ul>
</div>

<!-- MiniSearch (tiny client-side search engine) -->
<script src="https://unpkg.com/minisearch@6.3.0/dist/umd/index.min.js"></script>

<script>
(async function () {
  // fetch the prebuilt index (we'll generate this file in a sec)
  const res = await fetch('search-index.json');
  const docs = await res.json();

  const miniSearch = new MiniSearch({
    fields: ['title', 'content'],   // index these fields
    storeFields: ['title', 'url'],  // return these fields
  });
  ...
  function render(results) {
    ...
    countEl.textContent = results.length ? `${results.length} result(s)` : '';
  }

  q.addEventListener('input', () => {
    const query = q.value.trim();
    if (!query) { resultsEl.innerHTML = ''; countEl.textContent = ''; return; }
...
![](C:/Users/paige/OneDrive/Pictures/Screenshots/Screenshot 2025-01-11 183505.png)
