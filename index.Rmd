...
<div id="search-ui">
  ...
  <p id="status" style="margin-top:8px;color:#777;">Loading search...</p>
  <ul id="results" style="list-style:none;padding:0;"></ul>
</div>

<!-- Try multiple CDN sources for MiniSearch -->
<script>
// First try to load MiniSearch from CDN
...
// Backup: simple search without MiniSearch
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  try {
    ...
    useMiniSearch = true;
    statusEl.textContent = 'MiniSearch loaded successfully';
  } catch (e) {
    console.warn('MiniSearch failed to load, using simple search:', e);
    statusEl.textContent = 'Using simple search (MiniSearch unavailable)';
    useMiniSearch = false;
  }
  ...
  try {
    ...
    // Try multiple possible locations for the search index
    const possibleUrls = [
      'search-index.json',
      './search-index.json',
      '../search-index.json'
    ...
    if (!response || !response.ok) {
      throw new Error('Search index not found in any location');
    }
    
    searchDocs = await response.json();
    console.log(`Loaded ${searchDocs.length} documents from ${indexUrl}`);
    ...
    statusEl.textContent = `Ready to search ${searchDocs.length} pages`;
  ...
  function render(results) {
    if (!results || results.length === 0) {
      resultsEl.innerHTML = '';
      statusEl.textContent = qEl.value.trim() ? 'No matches found' : `Ready to search ${searchDocs.length} pages`;
      return;
    }
    
    resultsEl.innerHTML = results.slice(0, 8).map(r => `
      <li style="margin:10px 0;padding:10px;border:1px solid #ddd;border-radius:4px;background:#f9f9f9;">
        <a href="${r.url}" style="font-weight:bold;color:#0066cc;text-decoration:none;">${r.title}</a><br/>
