...
<script>
const INDEX_URL = "https://b33per.github.io/Statistics_Notebook/search-index.json"; // exact file
...
function render(results) {
  ...
  ul.innerHTML = results.map(r =>
    `<li style="margin:8px 0">
       <a href="${r.url}">${r.title || r.url}</a><br/><small>${r.url}</small>
     </li>`
  ).join('');
...
async function loadIndex() {
  // debug: show exactly what we’re fetching
  ...
  try {
    ...
    document.getElementById('status').textContent = `Indexed ${docs.length} page(s). Type & press Enter.`;
  } catch (e) {
    document.getElementById('status').textContent = `Could not load search index (${e.message}).`;
